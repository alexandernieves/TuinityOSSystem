...
export class IntelligenceService {
  ...
  async getReplenishmentSuggestions(tenantId: string) {
    ...
    const suggestions = products.map((product) => {
      const soldIn60Days = Number(
        saleItems.find((i) => i.productId === product.id)?._sum?.quantity || 0,
      );
      const ads = soldIn60Days / 60; // Average Daily Sales
      const currentStock = Number(
        inventory.find((i) => i.productId === product.id)?._sum?.quantity || 0,
      );
      ...
      if (daysToZero < settings.criticalDaysThreshold || currentStock === 0) {
        ...
        daysToZero < settings.warningDaysThreshold ||
  ...
  async getDeadStock(tenantId: string) {
    ...
    thresholdDate.setDate(
      thresholdDate.getDate() - settings.stagnantDaysThreshold,
    );
    ...
    for (const item of stockItems) {
      ...
      const lastMovementDate =
        lastSale?.sale?.createdAt || item.product.createdAt;
      ...
      if (daysInactive >= settings.stagnantDaysThreshold) {
        ...
        result.push({
          ...
          status:
            daysInactive >= settings.criticalDeadDaysThreshold
              ? 'CRITICAL_DEAD'
              : 'STAGNANT',
        });
      }
  ...
  async getPriceOptimizationSuggestions(tenantId: string) {
    ...
    for (const p of products) {
      const currentCost =
        Number(p.weightedAvgCost) || Number(p.lastCifCost) || 0;
      ...
      const currentMargin =
        lowestPrice === 0 ? 0 : (lowestPrice - currentCost) / lowestPrice;
      ...
      if (currentMargin < targetMargin && lowestPrice > 0) {
        ...
        suggestions.push({
          ...
          suggestedPrices: {
            ...
            price_b: Math.ceil(
              newBasePrice * (Number(p.price_b) / lowestPrice || 1.05),
            ),
            price_c: Math.ceil(
              newBasePrice * (Number(p.price_c) / lowestPrice || 1.1),
            ),
          },
        });
      }
  ...
  async getForecasting(tenantId: string) {
    ...
    return {
      ...
      projectionNextMonth:
        dataPoints.length > 0 ? dataPoints[dataPoints.length - 1][1] * 1.05 : 0,
    };
  }
  ...
  async processQuery(tenantId: string, query: string) {
    ...
    if (
      q.includes('comprar') ||
      q.includes('reabastecer') ||
      q.includes('falta') ||
      q.includes('bajo stock')
    ) {
    ...
    if (
      q.includes('no se vende') ||
      q.includes('estancado') ||
      q.includes('quedado') ||
      q.includes('muerto')
    ) {
    ...
    return {
      ...
      message:
        'No logré entender la consulta. Prueba con: ¿Qué productos me faltan? o ¿Cómo va mi margen?',
      data: null,
    };
  }
}

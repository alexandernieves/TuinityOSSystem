...
export default function CxCPashboardPage() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        const data = await api.getArSummary();
        setSummary(data);
      } catch (error) {
  ...
  const filteredBalances = summary?.customerBalances?.filter((c: any) => 
    c.customerId.toLowerCase().includes(searchTerm.toLowerCase()) ||
    summary.recentEntries.some((re: any) => re.customerId === c.customerId && re.customer.legalName.toLowerCase().includes(searchTerm.toLowerCase()))
  ) || [];

  // Note: the backend summary gives customerBalances which are the results of getCustomerBalance(id)
  // We need to match names. I'll probably need to adjust the backend to include legalName in summary.
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Cartera de Clientes (CxC)</h1>
          <p className="text-sm text-gray-500">Resumen global de cuentas por cobrar y morosidad.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-white dark:bg-[#141414] border-gray-200 dark:border-[#2a2a2a]">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Saldo Total CxC</p>
        ...
        <Card className="bg-white dark:bg-[#141414] border-gray-200 dark:border-[#2a2a2a]">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Índice de Morosidad</p>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                  {totalBalance > 0 ? ((totalOverdue / totalBalance) * 100).toFixed(1) : 0}%
                </h3>
              </div>
        ...
        <Card className="bg-white dark:bg-[#141414] border-gray-200 dark:border-[#2a2a2a]">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Clientes con Saldo</p>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mt-1">
        ...
        <CardContent>
          <div className="grid grid-cols-5 gap-4">
            {[
              ...
              const amount = summary.customerBalances.reduce((sum: number, c: any) => sum + Number(c.aging[period.key]), 0);
      ...
      <Card className="bg-white dark:bg-[#141414] border-gray-200 dark:border-[#2a2a2a]">
        <CardHeader className="flex flex-row items-center justify-between pb-4">
          <CardTitle className="text-lg font-semibold">Saldos por Cliente</CardTitle>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#222]">
                {summary.customerBalances.map((item: any) => {
                  const overdue = Number(item.aging.days30) + Number(item.aging.days60) + Number(item.aging.days90) + Number(item.aging.days90Plus);
                  return (
                    <tr key={item.customerId} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 font-bold text-xs">
                            {(item.customerName || 'CL').substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <p className="text-sm font-semibold text-gray-900 dark:text-white">{item.customerName || 'Cliente sin nombre'}</p>
                            <p className="text-xs text-gray-400">Código: {item.customerCode || 'N/A'}</p>
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link 
                          href={`/clientes/${item.customerId}?tab=cxc`}

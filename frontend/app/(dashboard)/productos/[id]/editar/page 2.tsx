  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setProduct(data);
        const getVal = (v: any, field: string = 'id') => {
          if (!v) return "";
          if (typeof v === 'string') return v;
          return v[field] || v.id || v.code || v.name || "";
        };

        const brandVal = getVal(data.brand, 'name');
        const groupVal = getVal(data.group, 'id');
        const supplierName = getVal(data.supplier, 'name');
        ...
        setFormData({
          ...
          supplier: supplierMatch?.id || supplierName || "",
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button onClick={() => router.push("/productos")} className="bg-[#2c5282] text-white hover:bg-[#1e3a5f]">
          Volver a Productos
        </Button>
      </div>
    );
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error("Error del servidor", {
          description: response.message || "No se pudo subir la imagen"
        });
      }
    } catch (err: any) {
      toast.error("Error de conexión", {
        description: "Asegúrate de que el servidor esté corriendo"
      });
      console.error(err);
    } finally {
    ...
    <div className="max-w-6xl mx-auto px-4 pb-12">
      ...
      <div className="bg-white rounded-[12px] border border-[#e1e3e5] shadow-[0_1px_3px_rgba(0,0,0,0.05)] overflow-hidden">
        <div className="p-8">
          ...
          <div className="mb-8 p-6 bg-[#f9fafb] rounded-[12px] border border-[#e1e3e5]/60">
